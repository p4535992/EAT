...
import util.object.model.GeoDocument;
import util.object.model.GeoDomainDocument;
import util.object.model.InfoDocument;
...
public class GeoDocumentDAO {
    ...
    public void addGeoDocument(GeoDocument geo) {
        ...
        try {
            trns = session.beginTransaction();
            SystemLogUtility.ticket("INSERIMENTO", "OUT");
            //System.out.println("---------------------------------------------------------------------------------------------------------------------------------------");
    ...
    public List<GeoDocument> getAllGeoDocuments(String nomeTabella,Integer LIMIT,Integer OFFSET) {
        ...
        try {
            ...
            String sql = "SELECT " 
                 ...
                 + "FROM "+nomeTabella+"";
            SystemLogUtility.ticket("HIBERNATE:" + sql + " LIMIT " + LIMIT + " OFFSET " + OFFSET + "", "AVOID");
    ...
    public GeoDocument getGeoDocumentById(String nomeTabella,String nameColumnID,String valueID) {
        ...
        try {
            ...
            SystemLogUtility.ticket("HIBERNATE:" + queryString + "", "AVOID");
        ...
        try{ 
            ...
            String sql;
             if(erase==true){
                 ...
                 //SQLQuery query = session.createSQLQuery(sql); 
                 SystemLogUtility.ticket("HIBERNATE:" + sql + "", "AVOID");
    ...
    public void GeoDocumentRemoveBlankSpaceBeforeAndAfterTheString(
         ...
         try {                    
            if(LIMIT == null && OFFSET ==null){
                ...
                SystemLogUtility.ticket(sql, "AVOID");
                query = session.createSQLQuery(sql).addEntity(GeoDocument.class);
                SystemLogUtility.ticket(query.getQueryString(), "AVOID");
    ...
    public boolean verifyDuplicate(String nomeTabella,String nameWhereColumn,String valueWhereColumn){
        ...
        try{
            ...
            if(results!=null && results.size() > 0){
                SystemLogUtility.ticket("Attenzione! Il record con " + nameWhereColumn.toString() + "='" + valueWhereColumn.toString() + "' è già presente nel database", "WAR");
                b=true;
            }
            tx =session.beginTransaction();
            tx.commit();
   ...
   public void addObjectAnnotated(Object obj,String pathPackageToObjectAnnotated){
      ...
      try{
         ...
         //session.getTransaction().commit();
         tx.commit();
          SystemLogUtility.ticket("E' stato creato un GeoDocument:" + obj.toString() + " nel database", "AVOID");
      }catch (HibernateException e) {
         if (tx!=null) tx.rollback();
         e.printStackTrace();  
      }catch(Exception e){
